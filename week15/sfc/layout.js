function getStyle (element) {
    ...
    for (let prop in element.computedStyle) {
        // let p = element.computedStyle.value; // fixme 这里有问题
...
function layout (element) {
    ...
    let items = element.children.filter(e => e.type === 'element'); // 排除非 element 节点（text 节点...）

    items.sort((a, b) => (a.order || 0) - (b.order || 0));  // NOTE: flex 的 order 属性
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (itemStyle.flex) {
            ...
            if (mainSpace < itemStyle[mainSize]) {
                ...
                flexLine = [item];  // ? 这里重新赋值了，上面两个属性不就没用了？？？
    ...
    // comput the cross sizes: align-items or align-slefs
    // let crossSpace;

    if (!style[crossSize]) {    // auto sizing
        crossSpace = 0;
        elementStyle[crossSign] = 0;
